...
case class CC(id:String, i1:Int, l2:Long,
              s3:String, e4:String, d5:String,
              t6:Long, a7:Seq[Int], a8:Seq[CC2] = Nil)


case class CC2(id:String, i1:Int, l2:Long,
               s3:String, e4:String, d5:String,
               t6:Long, a7:Seq[Int])
...
object TestPerformance {
  ...
  def generateSample:Effect[Unit] = {
    Effect.syncException({
      ...
      SparkTest.ss.range(1000000).repartition(48).map(longToSeed).map(genClass).write
        .mode(SaveMode.Overwrite).option("compression", "snappy").parquet("target/sample/randomNested")
    })
  }
